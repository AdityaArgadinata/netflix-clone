...
export function SearchResults({ query, results = [] }) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-zinc-100">
      ...
      <header className="sticky top-16 sm:top-20 z-40 border-b border-zinc-800 bg-black/95 px-6 py-4 sm:px-12 lg:px-16">
                ...
                return (
                  <Link key={`${item.id}-${item.type}`} href={href}>
                    <article className="group space-y-3 cursor-pointer">
                      {/* Poster */}
                      <div className="relative overflow-hidden rounded-lg border border-zinc-700 bg-zinc-900 transition group-hover:border-zinc-500 aspect-[2/3]">
                        {item.poster_path && !failedImages.has(item.id) ? (
                          ...
                          <>
                            <div className="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-b from-zinc-800 to-zinc-900 gap-2">
