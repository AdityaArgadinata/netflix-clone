...
export function SignInForm() {
  ...
  const oauthErrorMessage = oauthError ? decodeURIComponent(oauthError) : "";
  const displayErrorMessage = errorMessage || oauthErrorMessage;
  ...
  return (
    <>
      ...
      <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
        ...
        {displayErrorMessage ? (
          <p className="rounded-sm border border-red-500/40 bg-red-500/10 px-3 py-2 text-xs text-red-200">{displayErrorMessage}</p>
        ) : null}
